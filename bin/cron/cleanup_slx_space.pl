...
%runs = $dbc->Table_retrieve(
    $tables,
    [ 'Run_ID', 'Run_Directory', 'Run_Validation', 'Flowcell_Code','Lane','Run_DateTime'],
    , $condition
    ,-autoquote => 1
    );
...
my $rejected_condition = "WHERE SolexaRun.FK_Run__ID = Run.Run_ID and FK_Flowcell__ID = Flowcell_ID and Run_Validation in ('Rejected') and Files_Status = 'Raw' and Protected = 'No' and (TO_DAYS(CURDATE()) - TO_DAYS(Run_DateTime) > $rejected_threshold) and SolexaAnalysis_Finished is null";

$rejected_condition .= " limit 16";

%rejected_runs = $dbc->Table_retrieve(
    $rejected_tables,
    [ 'Run_ID', 'Run_Directory', 'Run_Validation', 'Flowcell_Code','Lane','Run_DateTime'],
    , $rejected_condition
    ,-autoquote => 1
...
foreach my $run_id ( @{ $runs{Run_ID} } ) {
    ...
    my ($protected_old) = $dbc->Table_find('Run,SolexaRun','Protected',"WHERE FK_Run__ID=Run_ID AND Run_Status IN ('Analyzed','Failed') AND Run_Validation IN ('Approved','Rejected') AND Files_Status = 'Raw' AND TO_DAYS(CURDATE())-TO_DAYS(Run_DateTime)>180 AND Protected = 'No' AND Run_ID=$run_id");
    ...
    } ## end if ( $check_analysis[10...
} ## end foreach my $run_id ( @{ $runs...
