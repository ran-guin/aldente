...
if (_check_block('Library')) {    
    ...
    # Get DBTable_IDs;
    my $sth = $dbc->query(-query=>"SELECT DBTable_ID,DBTable_Name FROM DBTable WHERE DBTable_Name in ('Ligation','Microtiter','Xformed_Cells','ReArray_Plate','Vector_Based_Library','Library')",-finish=>0);
    my $tids = &SDB::DBIO::format_retrieve(-sth=>$sth,-format=>'HofH',-keyfield=>'DBTable_Name');

    ## Get libraries that are missing FK_Vector_Based_Library__ID ##
    foreach my $format_table (@format_tables) {
	print ">>>Checking $format_table table...\n";
	my $primary = $format_table . "_ID";
	
	my $ids = $dbc->Table_retrieve(-table=>"$format_table",-fields=>[$primary],-condition=>"WHERE FK_Vector_Based_Library__ID IS NULL OR FK_Vector_Based_Library__ID = 0 OR FK_Vector_Based_Library__ID = ''",-format=>'CA');
	foreach my $id (@$ids) {
	    print ">$format_table ID $id missing FK_Vector_Based_Library__ID.\n";
	    # Now attempting to do the fix:
	    # First see if this is from a Submission
	    my $seq_lib_id = $dbc->Table_retrieve(-table=>"Submission_Detail AS ${format_table}_Sub, Submission_Detail AS Library_Sub, Vector_Based_Library"
						-fields=>['Vector_Based_Library_ID'],
						-condition=>"WHERE ${format_table}_Sub.FKSubmission_DBTable__ID = $tids->{$format_table}{DBTable_ID} AND ${format_table}_Sub.Reference = $id AND ${format_table}_Sub.FK_Submission__ID = Library_Sub.FK_Submission__ID AND Library_Sub.FKSubmission_DBTable__ID = $tids->{Library}{DBTable_ID} AND Vector_Based_Library.FK_Library__Name = Library_Sub.Reference",
						-format=>'S');	    
#
	    if ($seq_lib_id) {
		print ">Updating Vector_Based_Library_ID to $seq_lib_id based on submission_info.";
		my $ok = Table_update_array($dbc,$format_table,['FK_Vector_Based_Library__ID'],[$seq_lib_id],"WHERE $primary = $id",-autoquote=>1);
		if ($ok) {print " (OK)\n"}
		else {print " (Failed)\n"}
		next;
	    }
	...
	my $ids = $dbc->Table_retrieve(-table=>"Vector_Based_Library LEFT JOIN $format_table ON Vector_Based_Library_ID=FK_Vector_Based_Library__ID",-fields=>["Vector_Based_Library_ID"],-condition=>"WHERE Vector_Based_Library_Format='$format_names{$format_table}' AND FK_Vector_Based_Library__ID IS NULL",-format=>'CA');
	...
	foreach my $id (@$ids) {
	    ...
	    my @format_fields = ('FK_Vector_Based_Library__ID');
	    my @format_values = ($id);
