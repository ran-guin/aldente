...
use vars qw($Data_log_directory $testing $html_header %Configs $vector_directory);
##############################
# modular_vars               #
##############################
##############################
...
if ( $host eq $Configs{BACKUP_HOST} && $dbase eq $Configs{BACKUP_DATABASE} ) {
    print "This script does database updates, please run on production database.\n";
    exit;
}
...
## retrieve details for Stock to check up on (specified by Catalog Number ##
    my %NoticeInfo = $dbc->Table_retrieve(
        'Order_Notice',
        [ 'Order_Notice_ID as ID', 'Catalog_Number as Cat', 'Order_Text as Msg', 'Target_List as Target', 'Minimum_Units as Min', 'Maximum_Units as Max', 'Notice_Sent as Sent', 'Notice_Frequency as Freq', 'FK_Grp__ID as Grp' ],
        -date_format => 'SQL'
    );
    ...
    while ( defined $NoticeInfo{Cat}[$index] ) {
        ...
        my ($last_recvd) = $dbc->Table_find( 'Stock_Catalog', 'Stock_Catalog_Name,Stock_Type', "where Stock_Catalog_Number = '$cat_no' Order by Stock_Catalog_ID DESC LIMIT 1" );
        my ( $name, $type ) = split ',', $last_recvd;
        $type =~ s/Kit/Box/;
        $type =~ s/Reagent/Solution/;
        $type =~ s/Buffer/Solution/;
        $type =~ s/Primer/Solution/;
        $type =~ s/Matrix/Solution/;
        my %supplies = $dbc->Table_retrieve(
            "$type,Stock,Stock_Catalog",
            [ 'Stock_Catalog_Name', 'Stock_Catalog_Number', "${type}_Status", 'count(*) as count', 'max(Stock_Received) as Received', 'max(Stock_ID) as latestID', 'Stock.FK_Grp__ID as Owned_by' ],
            "where FK_Stock_Catalog__ID = Stock_Catalog_ID AND FK_Stock__ID=Stock_ID AND (Stock_Catalog_Number = '$cat_no' OR Stock_Catalog_Name = '$name') GROUP BY Stock_Catalog_Name,${type}_Status,Stock_Catalog_Number,FK_Grp__ID ORDER BY FK_Grp__ID,latestID DESC"
        );
        my $send          = 0;
        my $unopened      = 0;
        my $opened        = 0;
        ...
        while ( $supplies{Stock_Catalog_Name}[ ++$index ] ) {

            #my ($thisname,$thiscat,$status,$count,$received,$items) = split ',',$info;
            my $thisname = $supplies{Stock_Catalog_Name}[$index];
            ...
            my $allnames = join ',', $dbc->Table_find( 'Stock,Stock_Catalog', 'Stock_ID', "where FK_Stock_Catalog__ID = Stock_Catalog_ID AND Stock_Catalog_Name like '$thisname' AND Stock_Type = '$type'", 'Distinct' );
            my $allcats  = join ',', $dbc->Table_find( 'Stock,Stock_Catalog', 'Stock_ID', "where  FK_Stock_Catalog__ID = Stock_Catalog_ID AND Stock_Catalog_Number like '$thiscat'",                        'Distinct' );
            my $combos   = join ',',
                $dbc->Table_find(
                "Stock_Catalog,Stock,$type", 'Stock_ID',
                "where  FK_Stock_Catalog__ID = Stock_Catalog_ID AND FK_Stock__ID=Stock_ID AND Stock_Catalog_Number = '$thiscat' AND Stock_Catalog_Name like '$thisname' AND ${type}_Status = '$status'",
                -distinct => 1
                );
        ...
        if ( $unopened < $min ) {
            ...
            $msg = "<BR>Unopened <Font color=red><B>Supply Level Low !</B></Font> ($unopened < $min for $grp_name)<BR><B>$msg</B><BR>" . &Link_To( $home, '<- (edit this notification)', "&Search=1&Table=Order_Notice&Search List=$id" ) . "<BR>";
        ...
        if ($send) {
            ...
            my $ok = &alDente::Notification::Email_Notification(
                 $target, 'Supply Monitor<aldente@bcgsc.bc.ca>', $subject, "$notice\n$Table",
                -content_type => 'html',
...
if ( $Conflicts || $All ) {
    ...
    my @cat_conflicts = $dbc->Table_find( 'Stock,Solution,Stock_Catalog',
        "Stock_Catalog_Name", "WHERE FK_Stock_Catalog__ID = Stock_Catalog_ID AND  FK_Stock__ID=Stock_ID AND Solution_Status LIKE 'Unopened' Group by Stock_Catalog_Name having count(distinct Stock_Catalog_Number,FK_Organization__ID) > 1" );

    my @name_conflicts = $dbc->Table_find( 'Stock,Solution,Stock_Catalog',
        'Stock_Catalog_Number', "WHERE FK_Stock_Catalog__ID = Stock_Catalog_ID AND FK_Stock__ID=Stock_ID AND Solution_Status LIKE 'Unopened' Group by Stock_Catalog_Number having count(distinct Stock_Catalog_Name,FK_Organization__ID) > 1" );
    ...
    #	my $ok = $tmp->send_notification(-name=>"Stock Naming Conflicts",-from=>'Stock Name Consistency Checker<aldente@bcgsc.bc.ca>',-subject=>"Stock Naming Conflicts (from Subscription Module)",-body=>$Table,-content_type=>'html',-testing=>0,-bypass=>1);
    my $ok = alDente::Subscription::send_notification(
        -dbc          => $dbc,
        -name         => "Stock Naming Conflicts",
        -from         => 'Stock Name Consistency Checker<aldente@bcgsc.bc.ca>',
        -subject      => "Stock Naming Conflicts (from Subscription Module)",
        -body         => $Table,
        -content_type => 'html',
        -testing      => 0,
        -bypass       => 1
...
#### if not sent within 2 weeks (don't send now)
    ...
    if ( $send =~ /Using/ ) {
        ...
        my $ok = &alDente::Notification::Email_Notification(
             $target, 'Diagnostics Monitor<rguin@bcgsc.bc.ca>', 'Poor Runs Warning', "$msg",
            -content_type => 'html',
            -testing      => $dbc->test_mode()
        );
        ...
        my $ok = alDente::Subscription::send_notification(
            -dbc          => $dbc,
            -name         => "Poor Runs Warning",
            -from         => 'Diagnostics Monitor <rguin@bcgsc.ca>',
            -subject      => 'Poor Sequence Runs Warning (from Subscription Module)',
            -body         => $msg,
            -content_type => 'html',
            -testing      => $dbc->{test_mode}
        );
...
#    my $tmp = alDente::Subscription->new(-dbc=>$dbc);
#    my $ok = $tmp->send_notification(-name=>'Expiring Runs',-from=>'Notification Cron Job<rguin@bcgsc.bc.ca>',-subject=>"Expiring Test Runs $expired (Run ID: $old_test_runs) (from Subscription Module)",-body=>$msg,-content_type=>'html',-testing=>0,-bypass=>1);
        my $ok = alDente::Subscription::send_notification(
            -dbc          => $dbc,
            -name         => 'Expiring Runs',
            -from         => 'Notification Cron Job<rguin@bcgsc.bc.ca>',
            -subject      => "Expiring Test Runs $expired (Run ID: $old_test_runs) (from Subscription Module)",
            -body         => $msg,
            -content_type => 'html',
            -testing      => 0,
            -bypass       => 1
   ...
   #     my $ok = $tmp->send_notification(-name=>"Expiring Runs",-from=>'Notification Cron Job<rguin@bcgsc.bc.ca>',-subject=>"Expiring Production Runs $expired2 (Run ID: $old_production_runs) (from Subscription Module)",-body=>$msg,-content_type=>'html');
        my $ok = alDente::Subscription::send_notification(
            ...
            -name         => "Expiring Runs",
            -from         => 'Notification Cron Job<rguin@bcgsc.bc.ca>',
            -subject      => "Expiring Production Runs $expired2 (Run ID: $old_production_runs) (from Subscription Module)",
            -body         => $msg,
            -content_type => 'html'
        );
...
if ( $Integrity || $All ) {
    ...
    #    print "what's in admin seq: $Admins{'Sequencing'}\n";
...
###### Vector files exist for all vectors specified #######
    ...
    foreach my $vector (@vector_files) {
        ...
        unless ( %Vectors->{$file} ) {
            ...
            $Report->set_Warning("Found $name Vector, but '$file' NOT FOUND in Vector Directory ($vector_directory).");

            my @grps = $dbc->Table_find(
                'Library,LibraryVector,Vector,Vector_Type', 'FK_Grp__ID', "where FK_Vector_Type__ID = Vector_Type_ID and LibraryVector.FK_Vector__ID = Vector_ID and LibraryVector.FK_Library__Name=Library_Name AND Vector_Type_Name = '$name'",
                -distinct => 1,
                -debug    => 1
            );
    ...
    if ($msg) {
        &alDente::Notification::Email_Notification(
             $target, 'Integrity Monitor <aldente@bcgsc.bc.ca>', 'Strange Data', "$msg\n\nPlease ensure that this data is updated ASAP !!<BR>\n" . &Link_To( $home, "check record", "&Search=1&Table=Vector_Type&Search+List=$id" ),
            -content_type => 'html',
            -testing      => $dbc->test_mode()
        );
...
#    my $ok = $tmp->send_notification(-name=>"Missing Vector Sequence File",-from=>'Integrity Monitor <aldente@bcgsc.ca',-subject=>'Missing Vector Sequence File (from Subscription Module)',-body=>$msg."\n\nPlease ensure that this data is updated ASAP !!<BR>\n" . &Link_To($home,"check record","&Search=1&Table=Vector_Type&Search+List=$id"),-content_type=>'html',-testing=>0);
        my $ok = alDente::Subscription::send_notification(
            -dbc          => $dbc,
            -name         => "Missing Vector Sequence File",
            -from         => 'Integrity Monitor <aldente@bcgsc.ca',
            -subject      => 'Missing Vector Sequence File (from Subscription Module)',
            -body         => $msg . "\n\nPlease ensure that this data is updated ASAP !!<BR>\n" . &Link_To( $home, "check record", "&Search=1&Table=Vector_Type&Search+List=$id" ),
            -content_type => 'html',
            -testing      => 0
        );
...
#########################
    ...
    foreach my $label (@conflicts) {
        ...
        my %supplies = $dbc->Table_retrieve(
            ...
            [ 'Solution_ID', 'Stock_Catalog_Name', 'Stock_Catalog_Number', 'Solution_Status', 'count(*) as count', 'max(Stock_Received) as max', 'FK_Organization__ID', 'FK_Grp__ID AS Owned_By' ],
            "where FK_Stock_Catalog__ID = Stock_Catalog_ID AND FK_Stock__ID=Stock_ID AND (Stock_Catalog_Number = '$label' OR Stock_Catalog_Name = '$label') AND Solution_Status NOT LIKE 'Finished' GROUP BY Stock_Catalog_Name,Solution_Status,Stock_Catalog_Number,FK_Organization__ID,FK_Grp__ID"
        );
        my $send          = 0;
        my $unopened      = 0;
        my $opened        = 0;
        ...
        while ( $supplies{Solution_ID}[ ++$index ] ) {
            ...
            my $combos = join ',',
                $dbc->Table_find(
                'Stock,Solution,Stock_Catalog',                                                                                                                                                            'Stock_ID',
                "where FK_Stock_Catalog__ID = Stock_Catalog_ID AND FK_Stock__ID=Stock_ID AND Stock_Catalog_Number = '$thiscat' AND Stock_Catalog_Name like \"$thisname\" AND Solution_Status = '$status'", 'Distinct'
                );
            my $foundcombos = int( split ',', $combos );
            my $hide = 'Stock_Source,FK_Orders__ID,FK_Box__ID';    ## Hide fields

            my $name_link = &Link_To( $home, "$thisname", "&Edit+Table=Edit+Stock_Catalog+Table&Field=Stock_Catalog_Name&Like=$thisname&Hide=$hide",  'blue', ['newwin'] );
            my $cat_link  = &Link_To( $home, "$thiscat",  "&Edit+Table=Edit+Stock_Catalog+Table&Field=Stock_Catalog_Number&Like=$thiscat&Hide=$hide", 'blue', ['newwin'] );
