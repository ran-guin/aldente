...
use vars qw($opt_user $opt_password $opt_scope $opt_library $opt_project_id $opt_study_id $opt_run $opt_plate $opt_run $opt_well $opt_sample $opt_plate_number $opt_group $opt_key $opt_order $opt_limit $opt_quiet $opt_debug $opt_condition $Connection $opt_tables $opt_fields $opt_file $opt_add_fields $opt_list_fields $opt_xml $opt_include_empty_fields $opt_dbase $opt_host $opt_attribute $opt_alias $opt_alias_type $opt_alias_value $opt_type $opt_format $opt_plate_format $opt_include $opt_since $opt_until $opt_plate_type $opt_id $opt_barcode $opt_date_format $opt_rack $opt_flowcell $opt_solexa_run $opt_lane $opt_branch $opt_pipeline);
...
my $API        = Sequencing_API->new(-dbase=>$dbase,-host=>$host,-LIMS_user=>$user,-LIMS_password=>$password,-DB_user=>'viewer',-debug=>1);
my $Solexa_API = Solexa_API->new(-dbase=>$dbase, -host=>$host, -LIMS_user=>$user, -LIMS_password=>$password, -DB_user=>'viewer',-debug=>1);
...
push(@options,"-pipeline=>$pipeline") if $pipeline;
...
#     print Dumper($data) unless $debug;
    ...
    my $data = $API->get_Primer_data(-condition=>$condition,-add_fields=>$add_fields,-fields=>$fields,-custom=>1,-oligo=>1,-primer=>'MGC_27',
				     -quiet=>$quiet,-debug=>$debug,-limit=>$limit,-key=>$key,-format=>$format,-list_fields=>$list_fields,-save=>1,-verbose=>1);
    print Dumper($data) unless $debug;
}
    ...
    print q{my $data = define_Pool(-dbc=>$dbc,-full_name=>'Test Pool #1',-transposon_id=>1,-plate_id=>'50635',-wells=>['A01','A02','B01','B02','H12'],-date=>'1999-01-01',-name=>'TESTA',-pipeline=>'Standard',-gel_id=>22,-reads_required=>100,-status=>'Ready for Pooling',-goals=>'goals.');} . "\n\n";
	print "\n(similar example using multiple plates - note number of plates must match number of wells in this case)\n\n";
    print q{my $data = define_Pool(-dbc=>$dbc,-full_name=>'Test Pool #1',-transposon_id=>1,-plate_id=>['50635','50635','50635','50635','50635'],-wells=>['A01','A02','B01','B02','H12'],-date=>'1999-01-01',-name=>'TESTA',-pipeline=>'Standard',-gel_id=>22,-reads_required=>100,-status=>'Ready for Pooling',-goals=>'goals.');} . "\n\n";
    }
elsif ($scope =~ /solexa/) {
...
#    print "** 2 - Get only sizing info keyed on plate,well (for concentration run) **\n";
#    print q{my @fields = ('gel_datetime','gel_id','clone_band_sizes' ,'clone_size_estimate','sample_id');} . "\n\n";
#    print q{my $data =  $API->get_Clone_data(-dbc=>$dbc,-sample_id=>'763388,763394',-key=>"gel_plate,'-',sample_id",-fields=>\@fields,-quiet=>$quiet,-debug=>$debug);} . "\n\n";

 #   my @fields = ('gel_datetime','clone_size_estimate','clone_band_sizes','gel_id','sample_id');
 #   my $data =  $API->get_Clone_data(-dbc=>$dbc,-sample_id=>$sample,-key=>"gel_plate,'-',sample_id",-fields=>\@fields,-quiet=>$quiet,-debug=>$debug,-limit=>$limit);
 #   print Dumper($data) unless $debug;
 #}
