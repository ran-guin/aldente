...
use vars qw($Data_log_directory %Configs);
...
##### NEW CODE:
    ...
    my $dbc = SDB::DBIO->new( -dbase => $Configs{DATABASE}, -host => $Configs{SQL_HOST}, -user => 'viewer' );
    $dbc->connect();
    ...
    my $condition_string = "WHERE FK_Equipment__ID = Equipment_ID AND Equipment_Status = 'In Use' AND FK_Sequencer_Type__ID = Sequencer_Type_ID ";
    my $sequencer        = $opt_m;
    if ( $sequencer eq "all" ) {
        @machine_default_ids = &Table_find( $dbc, "Machine_Default,Equipment,Sequencer_Type", "Machine_Default_ID", "WHERE FK_Equipment__ID = Equipment_ID AND FK_Sequencer_Type__ID = Sequencer_Type_ID" );
    }
    elsif ( $sequencer =~ /.+/ ) {
        $sequencer =~ s/,/','/g;
        @machine_default_ids = &Table_find( $dbc, "Machine_Default,Equipment,Sequencer_Type", "Machine_Default_ID", "where FK_Equipment__ID = Equipment_ID AND  FK_Sequencer_Type__ID = Sequencer_Type_ID AND Host in ('$sequencer')" );
    ...
    my %Sequencer_Info = &Table_retrieve( $dbc, 'Machine_Default,Equipment,Sequencer_Type', ['Local_Data_Dir'], $condition_string );
    $dbc->disconnect();
    ...
    while ( defined %Sequencer_Info->{Local_Data_Dir}[$index] ) {
        ...
        # print preamble
        $Report->start_Section("Starting log for mirror cleanup $data_dir");
        ...
        $Report->end_Section("Starting log for mirror cleanup $data_dir");
        $Report->set_Message("Disk capacity report after  : $df[1]");
    }
...
###########################
    ...
    $Report->start_Section("Traverse $source directory");
    ...
    $Report->end_Section("Traverse $source directory");
    $Report->succeeded();
    return ( $source_element_count, $target_element_count, $removed_file_count, $failed_checksum_count );
}
...
#########################
    ...
    $Report->start_sub_Section("Clear out $file directory");
    ...
    unless ( $stamp =~ /2007/ ) {
        print "Unidentified File to move: $file\n";
        try_system_command("mv $file /home/sequence/mirror/unidentified/$year/$month/");
    }
    $Report->end_sub_Section("Clear out $file directory");
    return;
}
