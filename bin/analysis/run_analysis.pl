...
use vars qw($opt_help $opt_quiet $opt_host $opt_dbase $opt_user $opt_password $opt_reanalysis $opt_start_reanalysis $opt_run $opt_ignore $opt_analysis_mode $opt_analysis_comment $opt_implicit %Configs);
...
&GetOptions(
    ...
    'run'                => \$opt_run,
    'ignore=s'           => \$opt_ignore,
    'analysis_mode=s'    => \$opt_analysis_mode,
    'analysis_comment=s' => \$opt_analysis_comment,
...
my $analysis_mode    = $opt_analysis_mode;
my $analysis_comment = $opt_analysis_comment;

if ( $analysis_mode && !( $analysis_mode eq "Production" || $analysis_mode eq "Duplicate" || $analysis_mode eq "Test" ) ) { warn "Analysis mode must be Production, Duplicate or Test. AND First letter has to be in capital\n"; exit; }
...
my $current_processes = try_system_command("ps axwww | grep 'run_analysis.pl' | grep -v 'xemacs' | grep -v ' 0:00 ' | grep -v ' 0:01 ' | grep -v ' 0:02 '  | grep -v ' 0:03 '");
...
my %pipeline_version;
$pipeline_version{301} = '0.5.7';
$pipeline_version{303} = '0.7.4';
...
if ($reanalysis) {
    ...
    while (<TMP>) {
        ...
        if ( !$start_reanalysis && $implicit_concurrent_start ) {
    ...
    foreach my $run (@run_id) {
        ...
        if ( $analysis_type{$run_type} ) {
            ...
            foreach my $analysis_type (@analysis_types) {
                ...
                my $analysis_pipeline = &get_analysis_pipeline( -run_type => $run_type, -run_id => $run, -sub_run_type => $sub_run_type );
                my $run_analysis_obj;
                if ( $pipeline_version{$analysis_pipeline} ) {
                    $run_analysis_obj = $analysis_obj->new( -dbc => $dbc, -pipeline_version => $pipeline_version{$analysis_pipeline} );
                }
                else {

                    $run_analysis_obj = $analysis_obj->new( -dbc => $dbc );
                }
                if ( !$analysis_pipeline ) { print "No analysis pipeline\n"; next; }
                my $run_analysis_id = $run_analysis_obj->start_run_analysis( -run_id => $run, -analysis_pipeline_id => $analysis_pipeline, -analysis_mode => $analysis_mode, -analysis_comment => $analysis_comment, -force => 1 );
                if ( $run_analysis_id && $run_type eq 'SolexaRun' ) {
                    ...
                    $run_analysis_obj->create_multiplex_run_analysis( -run_analysis_id => $run_analysis_id, -analysis_mode => $analysis_mode );
                }
...
while ( $data_acquired->{Run_ID}[$index] ) {
    ...
    if ( $analysis_type{$run_type} ) {
        ...
        foreach my $analysis_type (@analysis_types) {
            ...
            if ( !$analysis_pipeline ) { print "No analysis pipeline\n"; next; }
            my $run_analysis_id = $run_analysis_obj->start_run_analysis( -run_id => $run_id, -analysis_pipeline_id => $analysis_pipeline, -analysis_mode => $analysis_mode, -analysis_comment => $analysis_comment );
            if ( $run_analysis_id && $run_type eq 'SolexaRun' ) {
                $run_analysis_obj->create_multiplex_run_analysis( -run_analysis_id => $run_analysis_id, -analysis_mode => $analysis_mode );
...
while ( $analyzing->{Run_ID}[$analyzing_index] ) {
    ...
    if ( $analysis_type{$run_type} ) {
        ...
        foreach my $analysis_type (@analysis_types) {
            ...
            if ( $pipeline_version{$analysis_pipeline} ) {
                $run_analysis_obj = $analysis_obj->new( -dbc => $dbc, -pipeline_version => $pipeline_version{$analysis_pipeline}, -id => $run_analysis_id, -base_directory => "" );
            }
            else {
                $run_analysis_obj = $analysis_obj->new( -dbc => $dbc, -id => $run_analysis_id, -base_directory => "" );
            }
...
sub get_analysis_pipeline {
    ...
    if ( $library_strategy eq 'Bisulfite-Seq' && $run_type eq 'SolexaRun' ) {
        if ( $project eq 'CEMT' ) {
            my $analysis_pipeline = 308;
            my ($analyzing) = $dbc->Table_find( "Run_Analysis", "COUNT(*)", "WHERE FKAnalysis_Pipeline__ID = $analysis_pipeline AND Run_Analysis_Status = 'Analyzing' AND FK_Run__ID != $run_id", -debug => 1 );
            if ( $analyzing > 0 ) {
                print "Pipeline 308 already has $analyzing analyzing\n";
                $analysis_pipeline = 0;
            }
	    $analysis_pipeline = 0;
    ...
    my ($miseq) = $dbc->Table_find( "Run,RunBatch,Equipment,Stock,Stock_Catalog,Equipment_Category", "Equipment_ID",
        "where FK_RunBatch__ID = RunBatch_ID and Run_ID = $run_id and FK_Equipment__ID = Equipment_ID and FK_Stock__ID = Stock_ID and FK_Stock_Catalog__ID = Stock_Catalog_ID and FK_Equipment_Category__ID = Equipment_Category_ID and Sub_Category = 'MiSeq'"
    );
    if ($miseq) {
        return 303;
    }
