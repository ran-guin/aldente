...
my %finished_runs = $dbc->Table_retrieve(
    "(Run,SolexaRun) LEFT JOIN Run_Analysis ON Run_Analysis.FK_Run__ID = Run_ID",
    [ 'Run_ID', 'Run_Type' ],
    "WHERE SolexaRun.FK_Run__ID = Run_ID AND ((Run_Status = 'In Process' AND Run_Validation = 'Pending' AND SolexaRun_Finished IS NOT NULL AND Run_Analysis_ID IS NULL) OR (Run_Analysis_Type = 'Primary' AND Run_Analysis_Status = 'Analyzing' AND FKAnalysis_Pipeline__ID = 300))"

        #"WHERE FK_Run__ID = Run_ID AND ((Run_Status = 'In Process' AND Run_Validation = 'Pending' AND SolexaRun_Finished IS NOT NULL) or Run_ID IN (122099,122100,122101,122102,122103,122104,122105,122106) )" #for testing
);
    ...
    if ( $analysis_type{$run_type} ) {
        ...
        foreach my $analysis_type (@analysis_types) {
            ...
            if ($run_analysis_id) {
                ...
                if ( !$run_analyzed ) {
                    $run_analysis_obj->check_expiring_analysis( -run_analysis_id => $run_analysis_id, -hour_limit => '24', -title => 'Expiring Primray Analysis Check' );
                }
            }
            else {
                ...
                if ( !$run_analysis_id ) {
                    ...
                    $run_analysis_obj->create_multiplex_run( -run_analysis_id => $run_analysis_id );
                }
            }
        }
    }
...
sub get_analysis_pipeline {
    ...
    $analysis_pipeline{'SolexaRun'}{'Single'} = 300;
    $analysis_pipeline{'SolexaRun'}{'Paired'} = 300;
