...
package alDente::Data_Validation;

use base SDB::Data_Validation;
...
my %Validate_condition = (
    Plate      => "Plate_Status IN ('Active','Temporary','Pre-Printed') AND Failed = 'No' AND QC_Status IN ('N/A','Passed','Pending','Re-Test')",   
    Microarray => " Microarray_Status IN ('Unused','Used') ",
);
...
sub get_aldente_id {
########################
    my %args = &filter_input( \@_, -args => 'dbc,barcode,table,limit,remove_repeats,feedback', -mandatory => 'dbc,table' );
    ...
    if ( $name || $pattern ) {                 ## search for the name of certain items to return an id.  (allow pattern (eg TL00*) ##
        if ( $table =~ /^(Employee|Contact|Organization|Project|Transposon)$/ ) {

            $condition .= " AND $table" . "_Name like '$pattern'";
            unless ($quiet) { print "SELECT $table" . "_ID FROM $table WHERE $condition\n"; }

            #	unless ($quiet) { print "Try: SELECT " . $table."_ID FROM $table $condition\n"; }
            my $found = join ',', $dbc->Table_find( $table, $table . "_ID", "WHERE $condition" );
            return $found;
        }
        else {
            $dbc->warning("get_aldente_id does not currently support searching for table: $table. (check with administrator if you need this functionality)\n");
            return;
        }
    }
    elsif ( $barcode || $table ) {    ## this variation returns id for standard table types (wrapper for SDB::get_id) ##
        ...
        if ( $validate && defined $Validate_condition{$TableName} ) {
            ...
            if ( $TableName =~ /Plate|Solution/i ) {
                if ($qc_check) {
                    $condition .= " AND QC_Status IN ('N/A','Passed')";
                }
                else {
                    $condition .= " AND QC_Status IN ('N/A','Passed','Pending','Re-Test')";
                }
            }
        }
