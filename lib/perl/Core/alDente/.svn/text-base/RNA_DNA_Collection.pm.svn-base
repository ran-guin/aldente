#!/usr/bin/perl
###################################################################################################################################
# RNA_DNA_Collection.pm
#
# Brief description
#
###################################################################################################################################
package alDente::RNA_DNA_Collection;
...
RNA_DNA_Collection.pm - !/usr/bin/perl
...
sub new {
    ...
    my $self = $this->alDente::Library::new(
        -dbc     => $dbc,
        -tables  => [ 'Library', 'Original_Source', 'RNA_DNA_Collection' ],
        -frozen  => $frozen,
        -encoded => $encoded
    );
...
#####################
    ...
    my $condition = "where FK_Project__ID = Project_ID and Library.Library_Name = RNA_DNA_Collection.FK_Library__Name";
    ...
    my %Lib_info = Table_retrieve( $dbc, 'Library,Project,RNA_DNA_Collection', \@field_list, $condition );
...
###################
    ...
    ## faster if library and project lists supplied up front ##
    ...
    #<CONSTRUCTION> This should be in Branch_App
    if ( param('New Valid Branch Codes') ) {
        my %layers;
        $layers{Primer} = &alDente::Branch::list_Branch_Codes( $dbc, -class => 'Primer' );
        $layers{Enzyme} = &alDente::Branch::list_Branch_Codes( $dbc, -class => 'Enzyme' );
    ...
    if ($admin) {
        @add_views_list = (
            ...
            #<CONSTRUCTION> This should call Branch_App
            Show_Tool_Tip(
                submit(
    ...
    return $self->SUPER::library_main(
        ...
        -labels       => { 'Library' => 'RNA/DNA Collection', },
...
################
    ...
    ###Libraries Section
    ##################################################################
    my $libraries = $self->_init_table('RNA_DNA_Collection');
...
$Id: RNA_DNA_Collection.pm,v 1.4 2004/11/06 01:09:24 echuah Exp $ (Release: $Name:  $)
