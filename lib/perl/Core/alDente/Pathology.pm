...
#####################
    ...
    my $self = $this->SDB::DB_Object::new( -dbc => $dbc, -tables => 'Pathology' );    ## if object IS a DB_Object (generally and class relating to database record)
    $self->{dbc} = $dbc;
    ...
    if ($id) {
        ...
        $self->primary_value( -table => 'Pathology', -value => $id );                 ## same thing as above..
        $self->load_Object();
    }
