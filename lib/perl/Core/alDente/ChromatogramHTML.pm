...
our ( $sequence_dbs_URL, $username, $project_dir, $bin_home );
use vars qw($mirror_dir $archive_dir $URL_domain);
##############################
# modular_vars               #
##############################
##############################
...
#
# ..?.. update this link (make generic or get from defaults directory..
#
#$sequence_dbs_URL = "http://seq.bcgsc.bc.ca/cgi-bin/rguin/dbsummary-front";
#
$username = "kteague";    # id of current maintainer
######################
...
sub ViewChromatogramApplet {
    ...
    my ( $dbc, $run_id, $well_id, $width, $height, $applet_only ) = @_;
...
HereSQL
        ...
        my $basepath = "$project_dir/$proj_path/$library_name/AnalyzedData/$sequence_subdir/chromat_dir/";
        ...
        if ( scalar @chrom_file > 0 ) {
            ...
            $chrom_file =~ s|^$project_dir/||;

            my $url_link;
            if    ( $linked_file =~ /\/mirror\/(.*)/ )  { $url_link = "/mirror/$1"; }
            elsif ( $linked_file =~ /\/archive\/(.*)/ ) { $url_link = "/archive/$1"; }
...
<tbody>
<tr><th align='right'>File: </th><td colspan='4'>&nbsp;$chrom_file<BR>->$url_link</td></tr>
...
BODY_1
            ...
            if ( -e "$project_dir/$chrom_file" ) {
                print "<B>found trace file ($project_dir/$chrom_file)...</B><P>";
            }
            else {
                print "<B>could not find $chrom_file - may be compressed</B><P>";
                print "- to decompress try typing the command:<B> $bin_home/decompress.pl -L $sequence_subdir</B> from a linux terminal window and reload this screen<P>";
            }

            my $applet_url_domain = $URL_domain;
            if ( $applet_url_domain =~ /^(http:\/\/(\w+))\./ ) {
                $applet_url_domain = $1;
            }

            print <<BODY_2;
<applet id='chromApp' name='chromApp' alt='$applet_alt $chrom_file' width='$width' height='$height'>
  <param name='file' value='$URL_domain/dynamic/data_home$url_link' valuetype='data' />
  <param name='codebase' value='$applet_url_domain/applets/' />
  <param name='code' value='Chromatogram_Applet.ChromatogramApplet' />
...
BODY_2
            ...
            if ( scalar @chrom_file > 1 ) {
                print "<p ></p>Multiple files found.</p>\n";
                foreach my $f (@chrom_file) {
                    print "$f<br>\n";
                }
                print "(Used first file found)";
            }
...
<body>
<p ></p><img src='http://olweb.bcgsc.bc.ca/intranet/images/WebScriptHelp.png' alt='Logo for WebScript help pages.' width='685' height='80' /></p>
...
<table width='100%'><col width='60' /><col /><tbody><tr><td>&nbsp;</td><td><p ></p>Generates an HTML page containing a java applet for viewing chromatographic traces.</p>
<p ></p>hello</p>
</td></tr></tbody></table>
</div>
...
$Id: ChromatogramHTML.pm,v 1.11 2003/11/27 19:37:57 achan Exp $ (Release: $Name:  $)
