...
############
    ...
    unless ($dbc) { $self->dbc->message("Error must supply connection"); return; }
...
###########L
    ...
    if ( @{ $self->errors } ) {
        foreach my $t_error ( @{ $self->errors } ) {
            ...
            #	    $self->dbc->message("ERROR: ($t_error)");
        }
    }
...
#############
    ...
    #    $self->dbc->message("Reset Transaction");
    #    Call_Stack();
    return;
}
...
###############
    ...
    if ( !@{ $self->{trans_names} } ) {
        ...
        $self->dbc->message("Warning closing $name: No transaction started") unless $force;    ## no warning (simply trying to force commit when nothing pending)

        #	Call_Stack();
        if ($force) { $self->dbc->message("Forced commit"); return $self->confirm_commit(); }
    }
    elsif ( $self->{trans_names}->[-1] ne $name ) {
        ## transaction is different from bottom transaction in stack ##
        $self->dbc->message("Warning closing $name: finishing $name transaction before $trans_names[-1]");
        ...
        #	$self->dbc->message("Finished $name; still pending: (@{$self->{trans_names}})");
        #	Call_Stack();
        if ($force) { $self->dbc->message("Forced commit"); return $self->confirm_commit(); }
    }
    else {
        ## only one transaction on stack ##
        #  $self->dbc->message("Committing $name") unless $quiet;
...
#############
    ...
    unless ( @{ $self->{trans_names} } ) {
        $self->dbc->message("Start $name transaction") if $debug;
        $self->reset(0);
    ...
    if ($debug) {
        ...
        my $prefix = '*' x $open;
        $self->dbc->message("$prefix Starting $name");    ## add bullets to enable tracking of depth of transaction
    }
...
sub finish {
    ...
    if ( int( @{ $self->{errors} } ) > 0 ) {
        ...
        if ( $self->{trans_names}->[-1] eq $name ) {
            ...
            $self->dbc->message("Aborted $name") if $debug;
        }
        elsif ( @{ $self->{trans_names} } ) {
            ## closed transaction doesn't match with top of stack ##
            $self->dbc->message("Warning: $name closed prematurely; pending: (@{$self->{trans_names}})");
            Call_Stack();
        }

        if ( $self->{rolled_back} ) {
            ## already rolled back... ##
            $self->dbc->message("Rollback includes $name") if $debug;
        }
        else {
            ## notify the original cause of the rollback problem
            $self->dbc->message("Rollback triggered from $name failure");
            $self->{rolled_back} = 1;
        ...
        unless ( $quiet || @{ $self->{trans_names} } ) {
            $self->dbc->message("Rolled back all $name transactions") if $debug;
        ...
        # SUCCESS #
        #       $self->dbc->message("Committing $name ($error)") if $debug;
    ...
    if ($debug) {
        ...
        my $prefix = '*' x $open;
        $self->dbc->message("$prefix Finished $name [$success]");
    }
