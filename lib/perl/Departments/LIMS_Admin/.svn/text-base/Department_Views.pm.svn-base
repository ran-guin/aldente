package LIMS_Admin::Department_Views;
...
#     $self->set_links($dbc);
    ...
    $page .= page_heading("LIMS Administration Home Page");

    my $show_access = $Access->View->display_DB_Access(); ## why doesn't this work !!  <CONSTRUCTION>
    
    my @layers;
    push @layers, { label=>'Database', content=> $self->View->manage_Database() };

    push @layers, { label=> 'Groups', content => $self->View->manage_Grps() . '<hr>' . $show_access };

    push @layers, { label=> 'Code Upgrade Releases', content => $self->View->code_Releases() };

    push @layers, { label=> 'Use Cases', content => $self->View->use_Cases() };
   
    push @layers, { label=> 'Encryption', content => $self->View->encryption() };

    push @layers, { label=> 'Email', content => $self->View->email() };
...
#######################
    ...
    my $lims_column = section_heading("LIMS Maintenance");
...
######################
    ...
    $overview .= $dbc->Table_retrieve_display(
        ...
        -link_parameters => {"DB_User" => "&cgi_app=LIMS_Admin::Department_App&rm=Edit Access&DB_User=<DB_User>"},
        -debug=>0,
        -return_html=>1,
    );
...
#####################
    ...
    my $hidden = $q->hidden(-name=>'cgi_app', -value=>'LIMS_Admin::Department_App', -force=>1) . $q->hidden(-name=>'rm', -value=>'Update Access', -force=>1);
...
#######################
    ...
    $this_layer .= 'Decode: '
    . $q->hidden(-name=>'cgi_app', -value=>'LIMS_Admin::Department_App', -force=>1)
