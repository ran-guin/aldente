#!/usr/bin/perl -w
###################################################################################################################################
# Reciving::Statistics_App.pm
#
#
#
# By Ash Shafiei, July 2008
###################################################################################################################################
package Receiving::Statistics_App;
...
###########################
    ...
    $form .= Views::sub_Heading( "Receiving Statistics", -1 );
    $form .= $q->hidden( -name => 'cgi_application', -value => 'Receiving::Statistics_App', -force => 1 );
...
###########################
    ...
    my @fields    = qw (Stock_ID Stock_Catalog_Name Stock_Number_in_Batch Stock_Received FK_Employee__ID FK_Orders__ID );
    ...
    my %values = Table_retrieve( $dbc, $tableName, \@fields, $condition, -distinct => $distinct, -condition => $cond );
