package Healthbank::Department_Views;
...
##############################
    ...
    ### Permissions ###
    my %Access = %{ $dbc->get_local('Access') } if $dbc->get_local('Access');

    my $datetime = &date_time;
    my $user     = $dbc->get_local('user_id');

    # This user does not have any permissions on Lab
    if ( !( $Access{'Healthbank'} || $Access{'LIMS Admin'} ) ) {
        return;
    }
    
    $self->Model->set_links();
    ...
    my $Views = $self->Model(-class=>'Views');

    $layers{'Sample Preparation'} = $Views->sample_prep_page();
    $layers{'AC Collection'}      = $Views->sample_prep_page( 'mobile' );
    $layers{'Administration'}     = $Views->_admin_page();
    $layers{'Export'}             = $Views->_export_page();
    $layers{'Search'}             = $Views->_search_page();
...
## Set plate fields
    ...
    foreach $type (@content_types) {
        my $png_dir =  $dbc->config('png_url_dir');
        my $label = "<img src='$png_dir/$type.png'/><BR>" . "$type";
        ...
        if ( $type eq 'Blood' || $type eq 'Saliva' ) {
            push( @links, &Link_To( $dbc->homelink(), $label, "&New+Entry=New+Source&Plate_Content_Type=$type$set$plateset&Grey=$grey&Omit=$omit&FK_Sample_Type__ID=$type_id" ) );
        }
        elsif ( $type eq 'Plasma' || $type eq 'Serum' ) {
            push( @links, &Link_To( $dbc->homelink(), $label, "&Database=healthbank&cgi_application=Healthbank::Statistics_App&rm=Display+Patient+Info&Original+Samples+Only=1" ) );
        }
