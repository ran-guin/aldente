...
#
package Healthbank::GENIC::Statistics_App;
...
use base Healthbank::Statistics_App;
...
###############
    ...
    my $summary = $dbc->Table_retrieve_display(
        'Library,Grp,Department,Plate,Plate_Sample,Sample,Source,Original_Source',
        ...
        "WHERE Plate.FK_Library__Name = Library_Name AND Library.FK_Grp__ID = Grp_ID AND Grp.FK_Department__ID = Department_ID AND Department_Name = 'GENIC' AND Plate_Sample.FKOriginal_Plate__ID=Plate.FKOriginal_Plate__ID AND Plate_Sample.FK_Sample__ID=Sample_ID AND Sample.FK_Source__ID=Source_ID AND Source.FK_Original_Source__ID=Original_Source_ID AND Plate_Status = 'Active'",
        -title       => 'Blood Collection Summary',
        -return_html => 1
    );

    my $summary2 = $dbc->Table_retrieve_display(
        'Library,Grp,Department,Plate,Plate_Sample,Sample,Source,Original_Source', [ 'Plate.FK_Plate_Format__ID', 'Plate.FK_Sample_Type__ID', 'count(distinct Plate_ID) as Active_Tubes', 'count(distinct Original_Source_Name) as Patients' ],
        "WHERE Plate.FK_Library__Name = Library_Name AND Library.FK_Grp__ID = Grp_ID AND Grp.FK_Department__ID = Department_ID AND Department_Name = 'GENIC' AND Plate_Sample.FKOriginal_Plate__ID=Plate.FKOriginal_Plate__ID AND Plate_Sample.FK_Sample__ID=Sample_ID AND Sample.FK_Source__ID=Source_ID AND Source.FK_Original_Source__ID=Original_Source_ID AND Plate_Status = 'Active' GROUP BY Plate.FK_Plate_Format__ID,Plate.FK_Sample_Type__ID",
    ...
    $output .= '<p ></p>' . Link_To( $homelink, ' Samples by Location',   "&cgi_application=alDente::View_App&rm=Display&File=/opt/alDente/www/dynamic/cache//Group/2/general/location_search.yml&Generate+Results=1&Source+Call=alDente::View_App" );
...
##################
    ...
    if (@sampletype) {
        push @conditions, "(" . join( ' OR ', map {"Source_Type = '$_'"} @sampletype ) . ")";
    }
...
######################
    ...
    for my $Source_ID ( sort { $a <=> $b } keys %IDs ) {
        ...
        my @plate_fields    = qw(Plate_ID Plate_Number Plate_Created Sample_Type.Sample_Type Plate_Status FK_Plate_Format__ID FK_Rack__ID );
...
##################
    ...
    $form .= &SDB::HTML::query_form(
        -dbc    => $dbc,
        -fields => [ 'Equipment.Equipment_Condition', 'Plate.Plate_Content_Type', 'Plate.Plate_Created', 'Plate.Plate_Status', 'Plate.FK_Rack__ID', 'Source.External_Identifier' ],
...
## Columns:
    ...
    my %values = $dbc->Table_retrieve( 'Source', [ 'Source_Type', 'Count(*) AS Count' ], "WHERE DATE(Received_Date) BETWEEN '$begin_date' AND '$end_date'", -group_by => 'Source_Type' );
