...
#
package Healthbank::Statistics_App;
...
use Healthbank::Statistics_Views;
...
##################
    ...
    $form .= &SDB::HTML::query_form(-dbc=>$dbc, -fields=>['Source.FK_Sample_Type__ID','Source.Received_Date','Source.Source_Status','Source.External_Identifier'],-title=>'Received Samples filter criteria',-action=>'search',-preset=>\%preset);
...
##################
    ...
    if (@sampletype) { push @conditions, "(" . join( ' OR ', map {"Sample_Type = '$_'"} @sampletype) . ")" }
...
# Received: # of sources received in the specified date range
    ...
    my %values = $dbc->Table_retrieve('Source,Sample_Type',
				      ['Sample_Type','Count(*) AS Count'],
				      "WHERE FK_Sample_Type__ID=Sample_Type_ID AND DATE(Received_Date) BETWEEN '$begin_date' AND '$end_date'",
