package alDente::Cancer_Genetics_Department;
...
my @icons_list = qw( Cancer_Genetics_Help Cancer_Genetics_Statistics Contacts Equipment);
...
##############################
  ...
  if (!($Access{'Cancer Genetics'} || $Access{'LIMS Admin'})) {
    return;
  }
...
## Set plate fields
  ...
  my $output;
  $output .= "<h2>Cancer Genetics Samples</h2>";
  $output .= '<hr>';
  ...
  foreach $type ( @content_types ) {
	  ...
	  push (@links, &Link_To($homelink,$label,"&Database=cancer_genetics&cgi_application=Cancer_Genetics::Statistics_App&rm=Display+Patient+Info&Original+Samples+Only=1"));
      }

  }
 
## Main table
  my $main_table = HTML_Table->new(-title=>"Cancer Genetics Home Page",
				   -width=>'100%', 
				   -bgcolour=>'white ',
				   -nolink=>1,
				   );  

  my $output = "<h2>Cancer Genetics Samples</h2>";
  $output .= '<hr>';
...
## Plates Section
  ...
  my $plates_box;
  if(grep(/Lab|Bioinformatics/,@{$Access{'Cancer Genetics'}})) {
    $plates_box = alDente::Department::plates_box(-type=>'Library_Plate',
						  -id_choices=>['-',					
								'View Ancestry',
								'Plate History',
							       ],
						  -access=>$Access{'Cancer Genetics'},
                                                  -include_rearray=>0,
						  -labels=>\%labels);
...
## Lab Layer
  ...
  my $libs = join "','", $dbc->Table_find('Library,Grp,Department','Library_Name',"WHERE FK_Department__ID=Department_ID AND Library.FK_Grp__ID=Grp_ID AND Department_Name IN ('Cancer Genetics')");
...
## Define the layers of the Cancer Genetics Department 
  my $layers = {
...
## Define admin layer
  my $admin_table = alDente::Admin::Admin_page($dbc,-reduced=>0,-department=>'Cancer Genetics',-form_name=>'Admin_layer');
...
##################
  ...
  # Cancer_Genetics permissions for searches
    if (grep(/Cancer_Genetics/,@{$Access{Cancer_Genetics}})) {
  ...
  # Admin permissions for searches
    if (grep(/Admin/,@{$Access{'Cancer Genetics'}})) {
