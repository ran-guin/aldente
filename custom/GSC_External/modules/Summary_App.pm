...
#
package GSC_External::Summary_App;
...
###############
    ...
    #     &GSC_External::auto_update_time_attributes($dbc);

    my $page;

    my $prompt = GSC_External::Views::prompt_for_summary( $dbc, -prefix=>$prefix, -layer=>$layer);

    #	    Link_To($homelink,'Regenerate Overview with Collection Times', "&cgi_application=GSC_External::App&rm=Summary&Include+Times=1");
    #    $page .= '<p ></p>' . Link_To($homelink,'Regenerate Overview without Collection Times', "&cgi_application=GSC_External::App&rm=Summary");
    #
    #
    ...
    if ( $q->param('rm') eq 'Generate Summary' ) {
        ...
        if ($prefix =~ /^BC/i ) {
            ...
            $page .= GSC_External::Views::overview( $dbc,  -include_times => $include_times, -source_condition => $source_condition, -plate_condition => $plate_condition, -debug => $debug );
        }
    }
    else {
        ## NOT regenerating summary ##
        $page .= GSC_External::Views::overview( $dbc, -prefix => $prefix ) . '<HR>' . $prompt;
    }
...
####################
    ...
    my $returnlink     = '&cgi_application=GSC_External::App&rm=Summary';
