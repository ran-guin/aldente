...
#
package GSC_External::Statistics_App;
...
###############
    ...
    my $summary = $dbc->Table_retrieve_display(
        'Plate,Plate_Sample,Sample,Source,Original_Source',
        [ 'count(distinct Plate_ID) as Active_Tubes', 'count(distinct Source_ID) as Vaccuum_Tubes_Drawn', 'count(distinct External_Identifier) as Blood_Tubes', 'count(distinct Original_Source_Name) as Patients' ],
        "WHERE Plate_Sample.FKOriginal_Plate__ID=Plate.FKOriginal_Plate__ID AND Plate_Sample.FK_Sample__ID=Sample_ID AND Sample.FK_Source__ID=Source_ID AND Source.FK_Original_Source__ID=Original_Source_ID AND Plate_Status = 'Active'",
        -title       => 'Blood Collection Summary',
        -return_html => 1
    );

    my $summary2 = $dbc->Table_retrieve_display(
        'Plate,Plate_Sample,Sample,Source,Original_Source', [ 'Plate.FK_Plate_Format__ID', 'Plate.FK_Sample_Type__ID', 'count(distinct Plate_ID) as Active_Tubes', 'count(distinct Original_Source_Name) as Patients' ],
        "WHERE Plate_Sample.FKOriginal_Plate__ID=Plate.FKOriginal_Plate__ID AND Plate_Sample.FK_Sample__ID=Sample_ID AND Sample.FK_Source__ID=Source_ID AND Source.FK_Original_Source__ID=Original_Source_ID AND Plate_Status = 'Active' GROUP BY Plate.FK_Plate_Format__ID,Plate.FK_Sample_Type__ID",
        -title       => 'Samples Available',
        -return_html => 1
    );

    my $output = $summary . '<hr>' . $summary2;
    $output .= '<p ></p>' . Link_To( $homelink, ' Samples by Subject ID', "&cgi_application=alDente::View_App&rm=Display&File=/opt/alDente/www/dynamic/cache//Group/2/general/subject_search.yml&Generate+Results=1&Source+Call=alDente::View_App" );
    $output .= '<p ></p>' . Link_To( $homelink, ' Samples by Content',    "&cgi_application=alDente::View_App&rm=Display&File=/opt/alDente/www/dynamic/cache//Group/2/general/content_search.yml&Generate+Results=1&Source+Call=alDente::View_App" );
...
sub old_home_page {
    ...
    my $home_form = start_custom_form( -name => 'Cancer Genetics Samples', -parameters => {&_alDente_URL_Parameters} );
...
##################
    ...
    #Start custom form
    my $form .= start_custom_form( -name => 'Search_Page', -parameters => { &_alDente_URL_Parameters() } );
    ...
    $form .= &SDB::HTML::query_form(
        -dbc    => $dbc,
        -fields => [ 'Source.FK_Sample_Type__ID', 'Source.Received_Date', 'Source.Source_Status', 'Source.External_Identifier' ],
        -title  => 'Received Samples filter criteria',
        -action => 'search',
        -preset => \%preset
    );
    ...
    ## Filter parameters
    my @sampletype = $q->param('FK_Sample_Type__ID');    ## Source.Sample_Type
...
##################
    ...
    my @tables         = qw(Source, Sample_Type);
    my @display_fields = qw(External_Identifier Received_Date Sample_Type FK_Patient__ID);
    my @conditions     = ('Source.FK_Sample_Type__ID=Sample_Type_ID');

    if (@sampletype) {
        push @conditions, "(" . join( ' OR ', map {"Sample_Type = '$_'"} @sampletype ) . ")";
    }
    ...
    if (@active) {
        push @conditions, "(" . join( ' OR ', map {"Source_Status = '$_'"} @active ) . ")";
    }
...
###################
    ...
    $form .= $q->submit(
        ...
        -value   => "Search By Patient ID",
        -class   => "Search",
        -force   => 1,
        -onClick => "unset_mandatory_validators(this.form); document.getElementById('Search_By_Patient_ID_validator').setAttribute('mandatory',1); return validateForm(this.form)"
        )
        . set_validator( -name => 'Source.FK_Patient__ID', -id => 'Search_By_Patient_ID_validator' )
        . &vspace();
...
######################
    ...
    my $tables    = 'Patient,Source,Sample,Plate_Sample,Plate,Sample_Type';
    my @fields    = qw(Source_ID Plate_ID);
    my $condition = "WHERE FK_Patient__ID = Patient_ID AND Plate.FK_Sample_Type__ID=Sample_Type_ID AND FK_Source__ID = Source_ID AND FK_Sample__ID = Sample_ID AND Plate_Sample.FKOriginal_Plate__ID = Plate_ID AND Patient_ID = $patient_id";
    if ($originals_only) {
    ...
    for my $Source_ID ( sort { $a <=> $b } keys %IDs ) {
        ...
        my @source_fields    = qw( FK_Sample_Type__ID Received_Date Collected_Date FK_Patient__ID);
        ...
        my @plate_fields    = qw(Plate_ID Plate_Number Plate_Created Sample_Type.Sample_Type Plate_Status Failed FK_Plate_Format__ID FK_Rack__ID );
...
##################
    ...
    $form .= &SDB::HTML::query_form(
        -dbc    => $dbc,
        -fields => [ 'Equipment.Equipment_Condition', 'Plate.Plate_Content_Type', 'Plate.Plate_Created', 'Plate.Plate_Status', 'Plate.Failed', 'Plate.FK_Rack__ID', 'Source.External_Identifier' ],
        -title  => 'Tube filter criteria',
        -action => 'search',
        -preset => \%preset
    );
    ...
    my @failed      = $q->param('Failed');                 ## Plate.Failed
...
###################
    ...
    my @display_fields = qw(Plate_ID Plate_Number Plate_Created Plate_Content_Type FK_Rack__ID FK_Pipeline__ID FK_Library__Name Plate_Status Failed);
    ...
    if (@sampletype) {
        push @conditions, "(" . join( ' OR ', map {"Plate_Content_Type = '$_'"} @sampletype ) . ")";
    }
    if ( $to_date || $from_date ) {
        if ( $to_date  && !$from_date ) { $from_date = $quick_link }
        if ( !$to_date && $from_date )  { $to_date   = $quick_link }
    ...
    if (@status) {
        push @conditions, "(" . join( ' OR ', map {"Plate_Status = '$_'"} @status ) . ")";
    }
    if (@failed) {
        push @conditions, "(" . join( ' OR ', map {"Failed = '$_'"} @failed ) . ")";
    }
    if (@location) {
...
######################
    ...
    my @conditions;
    if ($protocol) {
        push @conditions, "Lab_Protocol_Name = '$protocol'";
    }
    if ( $to_date || $from_date ) {
        if ( $to_date  && !$from_date ) { $from_date = $quick_link }
        if ( !$to_date && $from_date )  { $to_date   = $quick_link }
    ...
    if (@prep_action) {
        push @conditions, "(" . join( ' OR ', map {"Prep.Prep_Action = '$_'"} @prep_action ) . ")";
    }
...
## Columns:
    ...
    #select Sample_Type, Count(*) AS Count from Source Group By Sample_Type WHERE DATE(Received_Date) BETWEEN '$begin_date' AND '$enddate';
    my %values = $dbc->Table_retrieve( 'Source,Sample_Type', [ 'Sample_Type', 'Count(*) AS Count' ], "WHERE FK_Sample_Type__ID=Sample_Type_ID AND DATE(Received_Date) BETWEEN '$begin_date' AND '$end_date'", -group_by => 'Sample_Type' );

    my $table = $dbc->SDB::HTML::display_hash(
        -dbc   => $dbc,
        -keys  => [ 'Sample_Type', 'Count' ],
        -hash  => \%values,
...
## Columns:
    ...
    my %values = $dbc->Table_retrieve( 'Plate', [ 'Plate_Content_Type', 'Plate_Status', 'Count(*) AS Count' ], -group_by => 'Plate_Content_Type,Plate_Status', );
    ...
    my %minus80values = $dbc->Table_retrieve(
        'Plate,Rack',
...
## Columns:
    ...
    my %newvalues = $dbc->Table_retrieve(
        'Plate_Prep,Prep,Lab_Protocol',
