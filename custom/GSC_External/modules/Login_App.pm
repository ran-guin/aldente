...
##################
    ...
    my $project_name  = $q->param('Project') || $q->param('Target_Project');
    ...
    if ($retval) {
        ...
        my $view = new GSC_External::Login_Views(-dbc => $dbc);
        return $view->display_Login_page( -dbc => $dbc);
    }
    ...
    return $self->View->home_page(-user_name=>$user, -contact_id=>$contact_id, -project_name=>$project_name);
}
