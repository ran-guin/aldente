...
### Provide Link to Edit Preparation Tracking Settings (administrative permissions ) #### 
    print Link_To($homelink,"Edit Prep Tracking Settings",
...
### show detailed Library Preparation status only when Library chosen ###      
    if ($library && $details) {           
	print show_Prepped_Plates($dbc,$condition);
    }
    ...
    unless (@steps) {print "No Customized Prep Tracking steps setup for current department ($Current_Department).<br>(See LIMS admin to help set up this advanced functionality)"; return}
    ...
    else { print Views::Heading("$library Tubes") }
