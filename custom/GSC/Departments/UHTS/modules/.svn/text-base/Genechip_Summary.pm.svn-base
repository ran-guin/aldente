...
#
  ...
  $self->{config}{API_path}    = 'UHTS'; ## path to the API module
...
#
    ...
    $self->{config}{input_order} = [
				    ...
				    'GenechipRun.FKScanner_Equipment__ID'
				   ];
...
#
    ...
    $self->{config}{output_link} = {
				    'run_id' => "&HomePage=GenechipRun&ID=<VALUE>",
				   };
    ...
    if ($title =~ /expression/i){
      @extra_fields =('absent_probe_sets','absent_probe_sets_percent','algorithm','avg_a_signal','avg_p_signal','avg_m_signal','avg_background','avg_centralminus', 'avg_cornerminus', 'avg_cornerplus', 'avg_noise', 'avg_signal', 'controls', 'count_centralminus', 'count_cornerminus', 'count_cornerplus', 'marginal_probe_sets', 'marginal_probe_sets_percent', 'max_background', 'max_noise', 'min_background', 'min_noise', 'noise_rawq', 'present_probe_sets','present_probe_sets_percent','probe_pair_thr','std_background','std_noise','total_probe_sets','alpha1','alpha2','tau','noise_rawq','scale_factor','norm_factor','tgt');
    }elsif ($title =~ /mapping/i){
...
#
    ...
    $actions{1} = submit(-name=>'Set_Validation_Status',-value=>'Set Grid Alignment', -class=>'Action',-onClick=>"
    ...
    ## set Run.Billable
    $actions{2} = submit(-name=>'Set_Billable', -value=>'Set Billable', -class=>'Action')
                 . '&nbsp;'
    ...
    $actions{3} = submit(-name=>'Set_Artifact', -value=>'Set Artifact', -class=>'Action')
                 . '&nbsp;'
    ...
    $actions{4} = submit(-name=>'Set_as_Failed',-value=>'Set as Failed',-class=>'Action',-onClick=>"
        unset_mandatory_validators(this.form);
    ...
    $actions{5} = submit(-name=>'Set_Invoiced', -value=>'Set Invoiced', -class=>'Action')
                 . '&nbsp;'
...
#
  ...
  if(scalar @ids > 0){
	...
	$html->Set_Row([$snp, @{$snps_hash{$snp}{data}}]);
      ...
      require UHTS::GenechipRun;

      my $genechip_run = UHTS::GenechipRun->new(-dbc=>$dbc);
...
#
  ...
  if($title =~ /mapping/i){
    ...
    my @array = $dbc->Table_find("Probe_Set_Value, Probe_Set, GenechipExpAnalysis", "Probe_Set.Probe_Set_Name, Sig3, SigM, Sig5, Sig35", " where FK_GenechipExpAnalysis__ID = GenechipExpAnalysis_ID and FK_Run__ID = $key_field_value and FK_Probe_Set__ID = Probe_Set_ID and Probe_Set.Probe_Set_Name in ('AFFX-HUMGAPDH/M33197', 'AFFX-HSAC07/X00351')");
