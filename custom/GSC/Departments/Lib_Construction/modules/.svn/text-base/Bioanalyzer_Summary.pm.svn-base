package Lib_Construction::Bioanalyzer_Summary;
...
sub set_general_options {
    ...
    $self->{config}{API_path}   = 'Departments/Microarray';
...
################################
    ...
    $self->{config}{input_order} = [ 'Library.FK_Project__ID', 'Run.Run_DateTime', 'Run.Run_ID', 'Run.Run_Status', 'Run.Run_Validation', ];
...
#################################
    ...
    $self->{config}{output_order} = [ 'run_id', 'run_time', 'bioanalyzerrun_invoiced', 'billable', 'run_status', 'validation', ];
    return;
}
...
#################
    ...
    $actions{1} = submit( -name => 'Set_Billable', -value => 'Set Billable', -class => 'Action' ) . '&nbsp;' . popup_menu( -name => 'Billable', -values => [ $dbc->get_enum_list( 'Run', 'Billable' ) ], -default => 'Yes', -id => 'billable', -force => 1 );
